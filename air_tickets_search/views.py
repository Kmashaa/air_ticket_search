...
def flight_list_p(request) -> HttpResponse:
    ...
    if form.is_valid():
        ...
        flights = Flights.objects.filter(
            departure_city=departurecity, arrival_city=arrivalcity
        )
    return render(
        request,
        "air_tickets_search/flight_list.html",
        {
            "form": form,
...
def flight_list_bought(request) -> HttpResponse:
    ...
    return render(
        request, "air_tickets_search/flight_list_bought.html", {"flights": flights}
    )
...
def flight_search(request) -> HttpResponse:
    ...
    return render(
        request,
        "air_tickets_search/City_selection.html",
...
def flight_detail(request, fl) -> HttpResponse:
    ...
    if request.user.is_authenticated:
        flight_bought = Flights_bought.objects.create(
            id=fl,
        ...
        flight.delete()
        return render(
            request, "air_tickets_search/flight_detail.html", {"flight": flight_bought}
        )
    else:
        return render(
            request, "air_tickets_search/flight_detail.html", {"flight": flight}
        )
        ...
        if form.is_valid():
            ...
            if user is not None:
                if user.is_active:
                    ...
                    return render(
                        request,
                        "air_tickets_search/login_done.html",
                        {"flights": flights, "form": form},
                    )
                else:
                    return render(
                        request, "air_tickets_search/login.html", {"form": form}
                    )
            else:
...
def logout_view(request) -> HttpResponse:
    ...
    flights = Flights.objects.order_by("price")
    return render(
        request, "air_tickets_search/logout.html", {"flights": flights, "form": form}
    )
...
def register(request) -> HttpResponse:
    if request.method == "POST":
        ...
        if user_form.is_valid():
            ...
            return render(
                request, "air_tickets_search/register_done.html", {"new_user": new_user}
            )
    else:
